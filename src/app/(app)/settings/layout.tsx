                ...
                <div>
                  <Link
                    ...
                    className={`flex items-center justify-between px-3 py-2 rounded-xl text-xs font-semibold transition ${
                      isOverview
                        ? "bg-brand-50 text-brand-900 dark:bg-brand-950/60 dark:text-brand-300 font-bold border border-brand-200 dark:border-brand-800/80 shadow-2xs"
                        : "text-slate-600 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200 hover:bg-slate-100 dark:hover:bg-slate-900"
                    }`}
                  >
                    <span>Overview</span>
                    {isOverview && <ChevronRight className="w-3.5 h-3.5 text-brand-600" />}
                  </Link>
                </div>
                          ...
                          <li key={item.href}>
                            ...
                            >
                              <div className="flex items-center gap-2 truncate">
                                <Icon className={`w-4 h-4 shrink-0 ${isActive ? "text-brand-700 dark:text-brand-400" : "text-slate-400"}`} />
                                <span className="truncate">{item.name}</span>
                              </div>
                              {isActive && <ChevronRight className="w-3.5 h-3.5 text-brand-600 shrink-0" />}
                            </Link>
                          </li>
                        );
                      })}

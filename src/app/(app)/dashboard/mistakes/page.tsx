...
import { AlertTriangle, ChevronLeft, Play, BookOpen, Trash2 } from "lucide-react";

export default function MistakesPage() {
  const [mistakes, setMistakes] = useState<EngineQuestion[]>([]);
  const [isPracticing, setIsPracticing] = useState(false);

  useEffect(() => {
    try {
      const saved = JSON.parse(localStorage.getItem("mistake_bank") || "[]");
      setMistakes(saved);
    } catch {
      // ignore
    }
    ...
    return (
      <ExamRunner
        ...
        rules={{
          ...
          timeLimitMinutes: Math.max(10, Math.ceil(mistakes.length * 1.5)),
          passingScorePercentage: 80,
          allowsFlagging: true,
          hasContinuousTimer: true,
        }}
        title="Mistake Bank Practice Drill"
        subtitle={`Reviewing ${mistakes.length} previously missed items`}
      />
    );
  }

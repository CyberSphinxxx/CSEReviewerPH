...
export const testAttempts = pgTable(
  ...
  (table) => [
    index("idx_test_attempts_user_id").on(table.userId),
    index("idx_test_attempts_guest_session").on(table.guestSessionId),
    index("idx_test_attempts_exam_level").on(table.examLevelId),
  ]
);
...
export const userAnswers = pgTable(
  ...
  (table) => [
    index("idx_user_answers_attempt_id").on(table.testAttemptId),
    index("idx_user_answers_question_id").on(table.questionId),
  ]
);
...
export const userProgress = pgTable(
  ...
  (table) => [
    ...
    index("idx_user_progress_topic_id").on(table.topicId),
  ]
);
